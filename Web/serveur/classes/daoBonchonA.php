...
class DaoBouchonA
{
	...
	public function getlistBouchonAByIdArrivage($idArrivage)
	{
		...
		$rs=$this->dbh->prepare($query);
		$rs->bindParam(':idArrivage',$idArrivage);
		$rs->execute();
	...
	public function ajoutBouchonA($bouchonA)
	{
		...
		$tmp = $bouchonA;
		$rs=$this->dbh->prepare($query);
		$rs->bindParam(':idArrivage',$tmp->getIdArrivageBouchonA());
		$rs->bindParam(':long',$tmp->getLongueurBouchonA());
		$rs->bindParam(':dia1',$tmp->getDiametre1BouchonA());
		$rs->bindParam(':dia2',$tmp->getDiametre2BouchonA());
		$rs->bindParam(':diaComp',$tmp->getDiametreCompresseBouchonA());
		$rs->bindParam(':humi',$tmp->getHumiditeBouchonA());
	...
	public function updateBouchonA($bouchon)
	{
		...
		$rs->bindParam(':id',$tmp->getIdBouchonA());
		$rs->bindParam(':idArrivage',$tmp->getIdArrivageBouchonA());
		$rs->bindParam(':long',$tmp->getLongueurBouchonA());
		$rs->bindParam(':dia1',$tmp->getDiametre1BouchonA());
		$rs->bindParam(':dia2',$tmp->getDiametre2BouchonA());
